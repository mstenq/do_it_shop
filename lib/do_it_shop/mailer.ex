defmodule DoItShop.Mailer do
  defmacro __using__(_opts \\ []) do
    quote do
      use Swoosh.Mailer, otp_app: :do_it_shop
      import Swoosh.Email
      import DoItShop.Mailer
    end
  end
  @moduledoc false
  use Swoosh.Mailer, otp_app: :do_it_shop
  ...
  def render_body(email, template, args) when is_atom(template) and is_map(args) do
    heex = apply(DoItShopWeb.EmailHTML, template, [args])
    html_with_layout = render_component(DoItShopWeb.EmailHTML.layout(%{inner_content: heex}))
  ...
  def premail(email) do
    ...
    text_with_layout = render_component(DoItShopWeb.EmailHTML.layout_text(%{inner_content: text}))
  ...
  defp from_email, do: Application.get_env(:do_it_shop, :from_email) || "noreply@example.com"
end
