defmodule DoItShopWeb.CoreComponents do
  @moduledoc """
  ...
  def error(assigns) do
    ...
    <p class="mt-3 flex gap-3 text-sm leading-6 text-rose-600 phx-no-feedback:hidden dark:opacity-80">
      <.icon name="hero-exclamation-circle-mini" class="mt-0.5 h-5 w-5 flex-none dark:opacity-80" /> <%= render_slot(
        @inner_block
      ) %>
    </p>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DoItShopWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DoItShopWeb.Gettext, "errors", msg, opts)
    end
  end
